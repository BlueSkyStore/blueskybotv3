...
export default {
    methods: {
        ...
        /**
         * Choose file and folders
         */
        chooseAction() {
            var items = JSON.parse(JSON.stringify(this.selectedItems));
            var paths = _.map(items, a => a.path)
                .reduce((a, memo) => `${memo},${a}`, '');
            this.$store.dispatch('fm/urls', {
                disk: this.selectedDisk,
                paths: paths,
            }).then((response) => {
                if (response.data.result.status === 'success') {
                    ...
                    $(document).trigger('fm_file_select', [res]);
                }
            });
        }
    },
