    ...
    <b-card :title="$t('title')">
        <b-card-text>
            ...
            <b-modal v-model="editSenderModal.visible" :title="editSenderModal.title" size="xl" hide-footer>
                ...
                <w-json-editor
                    :schema="editSenderModal.model.jsonSchema"
                    v-model="editSenderModal.model.configurationJson"
                ></w-json-editor>
                <b-button
                    variant="success"
                    class="mt-3"
...
<script>
import loc from './EmailSenders.i18n.json';
    ...
    methods: {
        ...
        async save() {
            try {
                ...
                if (this.editSenderModal.isNew) {
                    // add
                    var data = await this.$api.post('/api/emailing/SmtpSender/Add', {
                        name: item.name,
                        type: item.type,
                    ...
                    // update
                    var data = await this.$api.post('/api/emailing/SmtpSender/Update', {
                        id: item.id,
                        name: item.name,
                        type: item.type,
                        description: item.description,
        ...
        async remove(item) {
            if (await this.toastAsk()) {
                try {
                    item.removing = true;
                    await this.$api.post('/api/emailing/SmtpSender/Delete', {
                        id: item.id
                    });
        ...
        async loadSenders() {
            ...
            try {
                var senders = await this.$api.get('/api/emailing/SmtpSender/GetAll');
                senders.forEach(s => {
        ...
        async loadExecutors() {
            ...
            try {
                var executors = await this.$api.get('/api/emailing/SmtpSenderExecutor/GetAll', null);
                executors.forEach(e => {
